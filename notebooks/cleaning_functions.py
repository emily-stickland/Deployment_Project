...
def label_encoding(df: pd.DataFrame):
    ...
    df_encoded = df[cols_to_encode].apply(le.fit_transform)
     ...
     # Assign the encoded values back to the original column names
    df_encoded.columns = cols_to_encode
...
def select_Xy_cols(df: pd.DataFrame):
    ...
    cols_to_inc =   df['Gender', 'Married', 'Education', 'Self_Employed', 'LoanAmount', \
